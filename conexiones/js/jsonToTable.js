...
function ConvertJsonToTable(parsedJson, tableId, tableClassName, linkText)
{
    ...
    if (parsedJson)
    {
        ...
        if(isStringArray)
            thCon += thRow.format('Vocabulario');
        else
        {
...
var nestedTable = [
  {
    ...
    for (let i = 0; i < json.length; i++) {
        for (let j = 1; j < 2; j++) {
           clues = clues.concat(json[i].easy[j].concat(json[i].medium[j]).concat(json[i].harder[j].concat(json[i].difficult[j])));
        }

    }
        let jsonTable = ConvertJsonToTable(clues.sort(), 'jsonTable', null, 'Download');


    let notes = json.length;

    document.getElementById("jsonTable").innerHTML = jsonTable;
